...
std::vector<std::shared_ptr<const RulePackage>> RulePackageCatalog::allPackages() const {
    std::vector<std::shared_ptr<const RulePackage>> result;
    result.reserve(static_cast<std::size_t>(packageCount_));
    for (auto it = packages_.cbegin(); it != packages_.cend(); ++it) {
        for (auto versionIt = it.value().cbegin(); versionIt != it.value().cend(); ++versionIt) {
            result.push_back(versionIt.value());
        }
    }
    std::sort(result.begin(), result.end(), [](const auto& a, const auto& b) {
        if (a->identity().packageId() != b->identity().packageId()) {
            return a->identity().packageId() < b->identity().packageId();
        }
        return a->identity().packageVersion() < b->identity().packageVersion();
    });
    return result;
}
