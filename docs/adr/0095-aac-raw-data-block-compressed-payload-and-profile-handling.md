...
| :--- | :--- | :--- |
| **1. ADTS Headers** | `tests/rules/aac_adts_analyzer_test.cpp:106-231` (`createsAnalyzerFromBundledPackageAndDecodesFieldsViaDsl`), `:2473-2600` (`decodesAdtsHeaderBitByBitRangesAndZeroLengthPayload`). | **Closed**: Full bit-by-bit coverage across all 18/19 fields (including indices 8–11 `original_copy`, `home`, `copyright_identification_bit`, `copyright_identification_start`), bit offsets 0..55/71, lengths, values, and zero-length payload frames (`raw_data_block` state=Materialized, length=0, 0 diagnostics). |
| **2. ASC / PCE** | `tests/rules/aac_adts_analyzer_test.cpp:629-1865` (`decodesAscCase1`..`7`, `rejectsAscCase8NonzeroAlignmentBit`, `rejectsAscCase9PrematureTruncation`), `:1960-2467` (AOT 5/29/39 non-GA suites), `:2606-2677` (`decodesAscFieldRangesRepresentativeSampling`). | **Closed (Representative Sampling)**: 9 full ASC test suites verifying 113–122 ordered child node names; representative field range and bit length assertions covering ASC header (`audio_object_type` [0,5), `sampling_frequency_index` [5,9), `channel_configuration` [9,13)), GASpecificConfig (`frame_length_flag` [13,14), `depends_on_core_coder` [14,15), `extension_flag` [15,16)), and PCE (`element_instance_tag` [16,20), `object_type` [20,22), `comment_field_bytes` [56,64)). Unsampled remainder: repetitive per-channel PCE elements. |
| **3. Stream Truncation** | `tests/rules/aac_adts_analyzer_test.cpp:290-323` (`handlesHeaderTruncationWithCrcPresent`), `:325-365` (`handlesPayloadTruncationAtEof`), `:379-475` (`materializesTruncatedTrailingFrameWhenRuleLacksPayloadDeclaration`), `:477-500` (`handlesTrailingGarbageSmallerThanHeader`), `:502-530` (`resynchronizesAcrossCorruptedByteSpan`), `:2682-2740` (`verifiesTruncatedFramesLogicalRangesAndDiagnosticLocations`). | **Closed**: Explicit `logicalRange()` assertions on truncated frame nodes ([0, 120) for 15-byte payload truncation, [0, 64) for 8-byte CRC header truncation) and diagnostic locations ([56, 120) for payload truncation, [56, 64) for header truncation). |
| **4. CRC Presence / Errors** | `tests/rules/aac_adts_analyzer_test.cpp:189-230` (`createsAnalyzerFromBundledPackageAndDecodesFieldsViaDsl` Frame 1), `:290-323` (`handlesHeaderTruncationWithCrcPresent`), `:2473-2600` (`decodesAdtsHeaderBitByBitRangesAndZeroLengthPayload`), `:2682-2740` (`verifiesTruncatedFramesLogicalRangesAndDiagnosticLocations`). | **Closed**: Explicit verification of `crc_check` field at bit 56..71 (16 bits, value `0x1234`), omitted when `protection_absent == 1`, and truncated header handling at bit 56..64 when CRC is incomplete. |
