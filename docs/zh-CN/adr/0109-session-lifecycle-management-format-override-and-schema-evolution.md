...
### 8. 会话持久化回归、Version 1 黄金兼容性与稀疏源恢复验证架构

为彻底实证会话全生命周期持久化、格式手动覆盖以及超大媒体源恢复在无损、无泄漏与低内存占用下的高可靠性，Task P6h 建立如下完备的验证矩阵与协议：

1. **固化 Version 1 黄金样例守护测试（`tests/fixtures/v1_golden.svsession`, P2-23）**：
   - 固化一个规范、封闭且包含完整字段的 Version 1 `.svsession` 文档（`tests/fixtures/v1_golden.svsession`）；
   - 样例覆盖规范定义的全部闭包字段：
     - `schemaVersion: 1`
     - `source`：文件源类型、规范化路径、字节大小与多段 SHA-256 指纹；
     - `rule`：精确规则包 ID、语义化版本号、内容哈希与入口点 ID；
     - `userState`：包含源位偏移的书签与包含位区间与文本的用户注释；
     - `viewState`：主光标选中位偏移与多层级树节点展开路径；
   - 编写永久回归守护测试，验证 `SessionDocument::parseJson()` 对此黄金样例的无损逐位解析，确保未来版本演进时对 Version 1 格式的绝对向后兼容性。

2. **100 GB 虚拟稀疏源会话恢复验证协议**：
   - 利用操作系统稀疏文件分配（seek 至 $100 \times 2^{30} - 1$ 字节并写入末尾字节，占用极微物理磁盘空间）构造 100 GiB 虚拟稀疏媒体源；
   - 实证大文件三段式边界指纹计算：准确计算首 1 MiB、中间 1 MiB 与尾部 1 MiB 的 SHA-256 摘要，而不将 100 GB 巨型文件载入内存；
   - 实证 100 GB 源的会话序列化与后续加载恢复，验证会话源身份匹配与指纹防篡改拦截，同时断言进程常驻内存（RSS）稳定处于默认预算（< 128 MiB）之内。

3. **格式手动覆盖全生命周期持久化回放协议**：
   - 端到端全链路自动化回放：
     - 打开存在格式歧义的媒体源（自动探测出多个候选格式）；
     - 触发手动覆盖，显式选择非默认规则重新解析；
     - 添加用户书签与展开节点，保存为 `.svsession` 会话文件；
     - 销毁当前会话，重新加载打开保存的 `.svsession` 文件；
     - 验证恢复后的会话直接绑定此前手动指定的格式规则，不再弹出歧义横幅或请求用户介入，同时完整还原书签、选中位与窗口未修改（clean）状态。

4. **未保存修改保护全交互分支自动化回放**：
   - 覆盖 `maybeSave()` 确认协议的全量决策分支：
     - 未修改会话：打开新文件或关闭窗口直接通过，不弹出多余提示；
     - 已修改会话选择保存（Save）：成功持久化修改并安全过渡；
     - 已修改会话选择放弃（Discard）：放弃未保存修改并安全过渡；
     - 已修改会话选择取消（Cancel）：中断操作并严格保留当前窗口与修改状态；
     - 保存文件对话框取消与只读 I/O 报错：弹出明确错误对话框并安全中止，绝对保护用户数据不丢失。

---

### 9. 阶段 6 任务拆分与依赖编排（P6a – P6i，P2-25）

为严格遵守项目纪律（单任务闭环 SOP、能力与消费者不混杂提交、固定独立评审门禁），阶段 6 划分为以下串行任务切片：

- **Task P6a**（规范编写与生命周期架构）：
