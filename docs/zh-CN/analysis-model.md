...
Cancellation source 使用 C++20 `std::stop_source` 与 `std::stop_token` 创建可复制
token。请求取消是线程安全且幂等的。消费者只在文档规定的取消点轮询 token；token
本身不会抛异常、强制终止工作或修改分析树。观察到请求的 worker 负责发布已完成
工作，并使用诊断把未完成节点标记为 cancelled。
