...
Status: In Progress
Current Phase: 5
Last Completed Step: Task P5j-4（含 P5j-4-R1、P5j-4-R2 两轮复审修订，`Required` 门禁已放行）— `AnalysisSession` 样本导航 API（`tracks()`、`samplesForTrack()`、`enterSample()` 与 `mdat` 坐标投影，格式判别全部由规则层 `targetFormat` 绑定驱动，`src/app/` 新增代码无编解码器名或 box 名分支）；R2 修订 P1-3：样本索引门控由 `finished()` 改为「容器树根为 `Materialized`」这一真不变量，终止失败经 `lastBatchStatus_` 透出 `SourceError`/`Cancelled`/`ResourceLimit`/`InvalidRulePackage` 而非压平成 `NotAnalyzed`
Next Action: Task P5j-5（样本导航的 UI 往返：FieldInspector、raw view 高亮与跨页选择）—— 但**须先完成检测优先级缺陷的独立受门禁切片**（见 Blockers），否则无法以真实 AVC-in-MP4 做 UI 验收
Last Verification: Task P5j-4-R2 — 修订 P1-3 后本机 Debug/Release/ASan-UBSan 三预设各 48/48 CTest 通过，sanitize 段扫描 `runtime error`/`AddressSanitizer`/`UndefinedBehaviorSanitizer` 命中 0 次；`AnalysisSessionTest` 44 个测试槽全通过（本轮新增 `tracksRefusesToAnswerFromATerminallyFailedContainer`）；P1-3 修订经反向变异实证承重：门控退回 `!finished()` 后新测试报 `Actual (tracks.status): 0` / `Expected (ResourceLimit): 14`，即从一棵 `finished()` 为真、根为 `Invalid`、却仍持有完整 `moov` 的废弃树上交出「看起来完全正常」的双轨列表并完成闭锁（状态 0 须穿过 `:826` 的 `sampleIndicesBuilt_ = true` 才可能返回，故该次实证同时覆盖「从废弃树上作答」与「闭锁真的发生」，补上了 R1 缺失的那一半）；同一次运行中既有 43 槽全部通过，证明该缺陷为既有测试集盲区；回滚后三文件与备份 SHA-256 逐字节一致，全仓无残留变异标记；生成器自洽性已验：临时目录重跑生成器与仓库内 5 个 fixture 逐个比对 SHA-256 全部 IDENTICAL
Blockers: 两项，均已实证、均非本片引入或本片不予修复。其一（阻塞 P5j-5）：检测优先级缺陷——真实 AVC-in-MP4 的 `moov` 字节会合成 Annex B 起始码（`VisualSampleEntry` 尾部 `... 00 00 | 00 01 | 0a`、`00 00 01 xx` 形态 box size、256–511 区间的 4 字节 NAL 长度前缀），使 H.264 达到 `Strong`；而 `chooseMp4` 要求 `h264Conf != Strong`，故单个 H.264`Strong` 即可一票否决一个 `Strong` 的 MP4，`svtool analyze` 对 `mp4_p5j4_avc_multi_nal.mp4` 实际按 H.264 NAL 解析。同一谓词在 `src/app/analysis_session.cpp:274-279` 与 `tools/svtool/main.cpp:150-152` 各存一份独立副本，会各自漂移；须单独立受门禁切片修复并把两处收敛为单一决策点（svtool 调用而非再抄），P5j-5 若以真实 AVC-in-MP4 做 UI 验收会在第一步失败。其二：现有测试对该路径结构性失明——`openPinnedMp4Fixture()` 经 `SessionDocument` 钉规则完全绕过仲裁，故「42 槽全绿」对检测正确性零信息量
