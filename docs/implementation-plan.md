...
Status: In Progress
Current Phase: 5
Last Completed Step: Task P5j-3b — 分析树 → `Mp4TrackSampleTables` 提取与 `Mp4SampleTableReaders` 绑定（`Mp4SampleTableExtractor`，规则层桥接切片）
Next Action: Task P5j-4（`AnalysisSession` 样本导航 API：`enterSample`、`samplesForTrack` 与 `mdat` 坐标投影）；P5j-6 为下一固定门禁
Last Verification: Task P5j-3b — 本机 Debug/Release/ASan-UBSan 三预设各 46/46 CTest 通过（新增 `streamview_mp4_sample_table_extractor_tests`）；`Mp4SampleTableExtractorTest` 12 个测试方法/14 个 QtTest 用例全通过，全部由 6 个新建完整轨道 fixture 端到端驱动（分析 → 提取 → `build()` → `descriptorPage()`），无任何手工表；两次反向变异实测可被捕获；sanitize 零报告
Blockers: None
...
- **Task P5j**（样本导航、时间线索引与收官切片）：
  ...
  - **Task P5j-3b**（规则桥接切片，P5j-4 准备阶段追加）：分析树 → `Mp4TrackSampleTables` 提取与 `Mp4SampleTableReaders` 绑定（把 `stsz`/`stco`/`co64`/`stss` 三个读取回调绑定到既有窗口解码器接缝，`stz2` field_size、`stsz.sample_size` 非零统一路径、`ctts` v0/v1 与 `stsd` `targetFormat` 绑定）；定位这些表必须依赖 box 类型或规则结构名判别，故只能位于规则层，且按「不把 capability 与第一个格式消费者塞进同一提交」独立成片；
...
- 2026-08-23：完成 Task P5j-3 —— 真实媒体 sample 的格式中立执行入口（`SamplePayloadFramer` 长度前缀分帧与 `SamplePayloadRunner` 样本子树执行，提交 `2d41c1bdd8e9cdfa2c4ee7fdd8da5b2ba5de6c01`）：
  ...
  2. AVC 长度前缀分帧恪守 ADR-0105 §4：`prefixLengthBytes` 由调用方从选中 sample description 的 `avcC.lengthSizeMinusOne + 1` 推导后传入，运行器只校验其 ∈ {1, 2, 4}（`isLegalPrefixLength`），非法值返回 `UnsupportedFraming` 而不猜测；一个 sample 内循环解析「L 字节前缀 + N 字节单元」直到样本边界，产出 `SamplePayloadUnitRange` 序列（含前缀与载荷各自的字节偏移与长度）。越界判定使用减法而非加法（`unitByteLength > sampleByteLength - payloadByteOffset`），从构造上避免整数溢出。
