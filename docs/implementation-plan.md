...
Status: In Progress
Current Phase: 5
Last Completed Step: Task P5j-3c — 样本配置摘要格式化（规则层 `ConfigurationSummaryRegistry` 能力片 + `AacAscConfigurationSummaryProvider` 消费者片，按 target format 键解析，`src/app/` 无需出现编解码器名）
Next Action: Task P5j-4（`AnalysisSession` 样本导航 API：`enterSample`、`samplesForTrack` 与 `mdat` 坐标投影），P5j-6 为下一固定门禁
Last Verification: Task P5j-3c — 本机 Debug/Release/ASan-UBSan 三预设各 48/48 CTest 通过；`ConfigurationSummaryTest` 13 个测试方法/15 个 QtTest 用例、`AacAscConfigurationSummaryProviderTest` 16 个测试方法/18 个 QtTest 用例全通过；能力片单独在树上实测 47/47 通过（消费者未接线时可独立编译与验证）；真实 fixture `mp4_p5h_mp4a_esds.mp4` 端到端产出 `AAC LC, 44100 Hz, stereo`；四次反向变异实测均被捕获——按名匹配配置结构被 9 条测试捕获，保留采样率索引猜 44100、escape AOT 丢失 +32 偏置、PCE 声道图猜 stereo 各被 1 条捕获，回滚后与验证态逐字节比对一致并转绿；sanitize 零报告
Blockers: None
